    ...
    return(
        ...
        <div className="w-full h-full flex flex-col overflow-hidden">
            <div className="flex flex-col gap-3 p-5">
                 <span className="text-[#2B6E36] font-semibold text-2xl">{exercicio.nome}</span>
                        <div className="flex gap-1">
                            <Link to={'/rotinas'} className="text-primary-green400 font-bold">Rotinas</Link>
                            <span className="text-primary-green400 font-extrabold">></span>
                            <Link to={`/rotinas/treino/${exercicio.fkTreino}`} className="text-primary-green400 font-bold">Treino {exercicio.fkTreino}</Link>
                            <span className="text-primary-green400 font-extrabold">></span>
                            <span className="text-primary-green400 font-bold">{exercicio.nome}</span>
                        </div>
            ...
            <div className="flex w-full h-3/5 max-h-3/5 py-5 gap-10 p-5">
                        <div className="w-3/5 p-5 bg-white shadow-sombra-padrao rounded-xl">
                            ...
                            <ReactPlayer url={exercicio.sourceUrl} controls={true} width='100%' height='100%'/>
                        ...
                        <div className="w-2/5 h-full flex flex-col  rounded-xl gap-5 justify-between items-center">
                                <div className="h-5/6  flex flex-col gap-5">
                                    ...
                                    <div className="h-full overflow-auto">
                                        <span className="text-lg text-justify ">{exercicio.descricao}</span>
                                    </div>
                                </div>
                                
                                {!exercicio.concluido ? (
                                    <Button content={'Marcar como concluído'}
                                    variant={'accept'}
                                    icon={<Check size={25} color={'#ffffff'} />}
                                    onClick={() => {handleClick(exercicio.id)}}/>
                                    ) : 
                                    <Button content={'Desmarcar como concluído'}
                                    variant={'decline'}
            ...
            <div className="flex w-full h-2/5 max-h-2/5 py-5 gap-10 p-5">
                        <div className="w-3/5 flex flex-col gap-2 bg-gray500 rounded-xl p-5">
                            ...
                            <span className="text-white text-xl font-semibold">Informações de execução</span>
                        ...
                        <div className="w-2/5 h-full flex flex-wrap rounded-xl p-5 gap-5">
                            {exercicio.tags && exercicio.tags.map((tag, index) => (
