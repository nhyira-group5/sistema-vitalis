...
export function MuralPage() {
    ...
    function getMuralItensByData(){
        ...
        try{
        api.get(`/murais/por-data/usuario/${loginResponse.id}`, { params })
        .then((response)=>{
    ...
    const deleteMuralItem = (itemId) => {
        ...
        api.delete(`/murais/${itemId}`)
            .then(()=>{
                ...
                setMuralItens(currentMuralItens => currentMuralItens.filter(item => item.idMural !== itemId));
            })
        } catch(error){
    ...
    function insertImage(cloudObject){
        ...
        try{
          api.post('/midias/salvarMidia', midiaDto)
          .then((response)=>{
